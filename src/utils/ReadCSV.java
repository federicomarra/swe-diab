...
public interface ReadCSV {
    static float[] ReadFloatCSV(String path) {
        ...
        try {
            ...
            System.out.println("READ:    Factor[h=" + firstlinesplit[0] + "] = " + firstlinesplit[1]); // print before first parsing
            ...
            System.out.println("WRITTEN: Factor[h=" + h + "] = " + units[h]);        // print after first parsing
            line = br.readLine();                                                   // reads second row
            while (line != null) {
                String[] linesplit = line.split(";");                               // separator in-row
                System.out.println("READ:    Factor[h=" + linesplit[0] + "] = " + linesplit[1]); // print before parsing
                try {
    ...
    static int[] ReadIntCSV(String path) {
        ...
        try {
            ...
            while (line != null) {  //returns a Boolean value
                ...
                System.out.println("READ:    Factor[h=" + linesplit[0] + "] = " + linesplit[1]);
                try {
                    h = Integer.parseInt(linesplit[0]) % 24;
                    units[h] = Integer.parseInt(linesplit[1]);
                    System.out.println("WRITTEN: Factor[h=" + h + "] = " + units[h]);
                } catch (NumberFormatException e) {
